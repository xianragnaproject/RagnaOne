...
sub received_characters {
	...
	# Do NOT pull state back to char-select once Map IP was received (state 4+)
	# or leftover 0x099D pages after HC_NOTIFY_ZONESVR will abort map login.
	my $st = $net->getState();
	if ($st < Network::CONNECTED_TO_CHAR_SERVER) {
		$net->setState(Network::CONNECTED_TO_LOGIN_SERVER) if $st != Network::CONNECTED_TO_LOGIN_SERVER;
	}
...
sub received_character_ID_and_Map {
	...
	# Drop unread char-list pages so they cannot re-enter received_characters
	# and reset state away from map login.
	$incomingMessages->clear() if $incomingMessages;
